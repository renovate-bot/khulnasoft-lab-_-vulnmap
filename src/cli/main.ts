...
function checkRuntime() {
  if (!runtime.isSupported(process.versions.node)) {
    ...
    console.error(
      'Please upgrade your Node.js runtime. The last version of Vulnmap CLI that supports Node.js v8 is v1.454.0.',
    );
  ...
  try {
    ...
    if (globalArgs.options['group-issues'] && globalArgs.options['iac']) {
      throw new UnsupportedOptionCombinationError([
        '--group-issues is currently not supported for Vulnmap IaC.',
      ]);
    }
