...
import { monitor as vulnmapMonitor } from '../../../lib/monitor';
import { processJsonMonitorResponse } from './process-json-monitor';
import vulnmap = require('../../../lib'); // TODO(kyegupov): fix import
...
images will be scanned by default when using the vulnmap container test/monitor
commands. If you are using Vulnmap in a CI pipeline, action may be required. Read
https://vulnmap.khulnasoft.com/blog/securing-container-applications-using-the-vulnmap-cli/ for
more info.`;
  ...
  for (const path of paths) {
    ...
    try {
      ...
      if (unsupportedPackageManager) {
        return `${unsupportedPackageManager.label} projects do not currently support "vulnmap monitor"`;
      }
      const targetFile =
        !options.scanAllUnmanaged && options.docker && !options.file // vulnmap monitor --docker (without --file)
          ? undefined
        ...
        try {
          if (!projectDeps.depGraph && !projectDeps.depTree) {
            ...
            throw new FailedToRunTestError(
              'Your monitor request could not be completed. Please email support@khulnasoft.khulnasoft.com',
            );
          }
...
function validateMonitorPath(path: string, isDocker?: boolean): void {
  ...
  if (!exists && !isDocker) {
    throw new Error('"' + path + '" is not a valid path for "vulnmap monitor"');
  }
}
