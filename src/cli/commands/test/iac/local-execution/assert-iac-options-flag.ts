...
const integratedKeys: (keyof IaCTestFlags)[] = ['vulnmap-cloud-environment'];
...
export class FlagError extends CustomError {
  constructor(key: string) {
    ...
    const msg = `Unsupported flag "${flag}" provided. Run vulnmap iac test --help for supported flags`;
    super(msg);
...
export class IntegratedFlagError extends CustomError {
  constructor(key: string, org: string) {
    const flag = getFlagName(key);
    const msg = `Flag "${flag}" is only supported when using ${IacV2Name}. To enable it for your organisation "${org}", please contact Vulnmap support.`;
    super(msg);
    this.code = IaCErrorCodes.FlagError;
    this.strCode = getErrorStringCode(this.code);
    this.userMessage = msg;
...
export class FeatureFlagError extends CustomError {
  constructor(key: string, featureFlag: string, hasVulnmapPreview?: boolean) {
    const flag = getFlagName(key);
    let msg;
    if (hasVulnmapPreview) {
      msg = `Flag "${flag}" is only supported if feature flag '${featureFlag}' is enabled. The feature flag can be enabled via Vulnmap Preview if you are on the Enterprise Plan`;
    } else {
      msg = `Flag "${flag}" is only supported if feature flag "${featureFlag}" is enabled. To enable it, please contact Vulnmap support.`;
    }
    super(msg);
...
export class UnsupportedEntitlementFlagError extends CustomError {
  constructor(key: string, entitlementName: string) {
    ...
    this.strCode = getErrorStringCode(this.code);
    this.userMessage = `Flag "${flag}" is currently not supported for this org. To enable it, please contact vulnmap support.`;
  }
}
...
export class UnsupportedEntitlementCommandError extends CustomError {
  constructor(key: string, entitlementName: string) {
    ...
    this.code = IaCErrorCodes.UnsupportedEntitlementFlagError;
    this.strCode = getErrorStringCode(this.code);
    this.userMessage = `Command "${key}" is currently not supported for this org. To enable it, please contact vulnmap support.`;
  }
}

/**
 * Validates the command line flags passed to the vulnmap iac test
 * command. The current argument parsing is very permissive and
 * allows unknown flags to be provided without validation.
 *
 * For vulnmap iac we need to explicitly validate the flags to avoid
