...
import {
  ...
  getTrees,
  parsePath,
} from '@khulnasoft/cloud-config-parser';
  ...
  const formattedIssues = scanResult.violatedPolicies.map((policy) => {
    ...
    return {
      ...
      documentation: !isGeneratedByCustomRule
        ? `https://security.vulnmap.khulnasoft.com/rules/cloud/${policy.publicId}`
        : undefined,
...
export class FailedToFormatResults extends CustomError {
  constructor(message?: string) {
    ...
    this.userMessage =
      'We failed printing the results, please contact support@khulnasoft.khulnasoft.com';
  }
}
