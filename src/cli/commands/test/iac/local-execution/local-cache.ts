...
const cachePath = config.CACHE_PATH ?? envPaths('vulnmap').cache;
...
export class FailedToDownloadRulesError extends CustomError {
  constructor(message?: string) {
    ...
    this.strCode = getErrorStringCode(this.code);
    this.userMessage =
      'We were unable to download the security rules, please ensure the network can access https://static.vulnmap.khulnasoft.com';
  }
}
...
export class FailedToExtractCustomRulesError extends CustomError {
  constructor(path: string, message?: string) {
    ...
    this.strCode = getErrorStringCode(this.code);
    this.userMessage = `We were unable to extract the rules provided at: ${path}. The provided bundle may be corrupted or invalid. Please ensure it was generated using the 'vulnmap-iac-rules' SDK`;
  }
}
...
export class InvalidCustomRules extends CustomError {
  constructor(path: string, message?: string) {
    ...
    this.strCode = getErrorStringCode(this.code);
    this.userMessage = `We were unable to extract the rules provided at: ${path}. The provided bundle does not match the required structure. Please ensure it was generated using the 'vulnmap-iac-rules' SDK`;
  }
}
