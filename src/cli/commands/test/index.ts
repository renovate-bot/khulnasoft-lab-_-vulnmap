...
import { FailOn } from '../../../lib/vulnmap-test/common';
import {
...
import {
  getPackageJsonPathsContainingVulnmapDependency,
...
const debug = Debug('vulnmap-test');
const SEPARATOR = '\n-------------------------------------------------------\n';
...
export default async function test(
  ...
  const packageJsonPathsWithVulnmapDepForProtect: string[] = getPackageJsonPathsContainingVulnmapDependency(
    options.file,
    paths,
  );

  analytics.add(
    'upgradable-vulnmap-protect-paths',
    packageJsonPathsWithVulnmapDepForProtect.length,
  );
  ...
  for (const path of paths) {
    ...
    try {
      res = await vulnmap.test(path, testOpts);
    } catch (error) {
    ...
    if (options.failOn) {
      ...
      if (!fail) {
        ...
        response += getProtectUpgradeWarningForPaths(
          packageJsonPathsWithVulnmapDepForProtect,
        );
