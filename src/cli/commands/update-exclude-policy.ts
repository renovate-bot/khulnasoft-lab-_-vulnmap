...
import * as vulnmapPolicyLib from 'vulnmap-policy';
...
export default async (...args: MethodArgs): Promise<any> => {
  ...
  // Ensure that this update-exclude-policy command can only be runned when using `vulnmap iac update-exclude-policy`
  // Avoid `vulnmap update-exclude-policy` direct usage
  if (options.iac != true) {
  ...
  try {
    ...
    let policy: Policy;
    try {
      policy = await vulnmapPolicyLib.load();
    } catch (error) {
      if (error.code === 'ENOENT') {
        // policy file does not exist - create it
        policy = await vulnmapPolicyLib.create();
      } else {
        throw error;
      }
    }
    await updateExcludeInPolicy(policy, analysis, options);
    await vulnmapPolicyLib.save(policy);
  } catch (e) {
