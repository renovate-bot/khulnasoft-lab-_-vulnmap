...
const DEBUG_DEFAULT_NAMESPACES = [
  ...
  'vulnmap:run-test',
  'vulnmap:prune',
  'vulnmap-nodejs-plugin',
  'vulnmap-gradle-plugin',
  'vulnmap-sbt-plugin',
  'vulnmap-mvn-plugin',
  'vulnmap-yarn-workspaces',
];
...
export interface ArgsOptions {
  ...
  // (see the vulnmap-mvn-plugin or vulnmap-gradle-plugin)
...
export function args(rawArgv: string[]): Args {
  ...
  const debug = debugModule('vulnmap');
  ...
  // an argument to our command, like `vulnmap test package2test`
  ...
  // vulnmap [mode?] [command] [paths?] [options-double-dash]
  ...
  // alias `-v` to `vulnmap version`
  if (argv.version) {
    command = 'version';
  }

  // alias `--about` to `vulnmap about`
  if (argv.about) {
    command = 'about';
  }

  if (!command || argv.help || command === 'help') {
    // bit of a song and dance to support `vulnmap -h` and `vulnmap help`
    ...
    // so that no argument gets deleted or ignored. This ensures `vulnmap --help [command]` and `vulnmap [command] --help` return the
    // specific help page instead of the generic one.
    // This change also covers the scenario of 'vulnmap [mode] [command] --help' and 'vulnmap --help [mode] [command]`.
    if (!argv._.length) {
  ...
  if (command && command.indexOf('config:') === 0) {
    ...
    // vulnmap.config('set', 'api=x')
    // vulnmap.config('get', 'api') // etc
