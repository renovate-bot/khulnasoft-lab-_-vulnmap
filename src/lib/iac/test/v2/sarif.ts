...
export function convertEngineToSarifResults(scanResult: TestOutput): sarif.Log {
  ...
  const tool: sarif.Tool = {
    driver: {
      name: 'Vulnmap IaC',
      fullName: 'Vulnmap Infrastructure as Code',
      version: getVersion(),
      informationUri:
        'https://docs.vulnmap.khulnasoft.com/products/vulnmap-infrastructure-as-code',
    ...
    runs: [
      {
        ...
        results: mapVulnmapIacTestResultsToSarifResults(scanResult.results),
      },
    ],
  };
}
...
function extractReportingDescriptor(
  ...
  results.vulnerabilities.forEach((vulnerability) => {
    ...
    const tags = ['security']; // switch to rules.labels once `vulnmap-iac-test` includes this info
...
function mapVulnmapIacTestResultsToSarifResults(
  results: Results | undefined,
): sarif.Result[] {
