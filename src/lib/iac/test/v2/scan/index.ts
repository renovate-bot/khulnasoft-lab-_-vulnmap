...
import { mapVulnmapIacTestOutputToTestOutput, TestOutput } from './results';
...
const debug = newDebug('vulnmap-iac');
const debugOutput = newDebug('vulnmap-iac:output');

export const systemCachePath = config.CACHE_PATH ?? envPaths('vulnmap').cache;
...
async function scanWithConfig(
  ...
  if (apiUrl.startsWith('http://')) {
    console.warn(
      '\nYou configured the Vulnmap CLI to use an API URL with an HTTP scheme. This option is insecure and might prevent the Vulnmap CLI from working correctly.',
    );
  }

  env['VULNMAP_IAC_TEST_API_REST_URL'] =
    process.env['VULNMAP_IAC_TEST_API_REST_URL'] || apiUrl;
  env['VULNMAP_IAC_TEST_API_REST_TOKEN'] =
    process.env['VULNMAP_IAC_TEST_API_REST_TOKEN'] || getApiToken();
  env['VULNMAP_IAC_TEST_API_REST_OAUTH_TOKEN'] =
    process.env['VULNMAP_IAC_TEST_API_REST_OAUTH_TOKEN'] || getOAuthToken();
  env['VULNMAP_IAC_TEST_API_V1_URL'] =
    process.env['VULNMAP_IAC_TEST_API_V1_URL'] || apiUrl;
  env['VULNMAP_IAC_TEST_API_V1_TOKEN'] =
    process.env['VULNMAP_IAC_TEST_API_V1_TOKEN'] || getApiToken();
  env['VULNMAP_IAC_TEST_API_V1_OAUTH_TOKEN'] =
    process.env['VULNMAP_IAC_TEST_API_V1_OAUTH_TOKEN'] || getOAuthToken();
  ...
  if (debugOutput.enabled) {
    debugOutput('vulnmap-iac-test output:\n', JSON.stringify(results, null, 2));
  }

  return mapVulnmapIacTestOutputToTestOutput(results);
}
...
function processFlags(
  ...
  if (options.vulnmapCloudEnvironment) {
    flags.push('-vulnmap-cloud-environment', options.vulnmapCloudEnvironment);
  }
...
async function createTemporaryFiles(
  ...
  try {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), 'vulnmap-'));
    const tempOutput = path.join(tempDir, 'output.json');
    const tempPolicy = path.join(tempDir, '.vulnmap');
