import { SEVERITY } from '../../../../vulnmap-test/common';
import { IacProjectType } from '../../../constants';
import { VulnmapIacTestError } from '../errors';
...
export function mapVulnmapIacTestOutputToTestOutput(
  vulnmapIacOutput: VulnmapIacTestOutput,
): TestOutput {
  const entitlementError = vulnmapIacOutput.errors?.find(
  ...
  const readSettingsError = vulnmapIacOutput.errors?.find(
  ...
  return {
    results: vulnmapIacOutput.results,
    settings: vulnmapIacOutput.settings,
    errors,
    warnings,
  };
}
...
export interface TestOutput {
  results?: Results;
  errors?: VulnmapIacTestError[];
  warnings?: VulnmapIacTestError[];
  settings: Settings;
}

export interface VulnmapIacTestOutput {
  results?: Results;
