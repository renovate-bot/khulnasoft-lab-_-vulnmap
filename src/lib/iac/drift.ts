...
import vulnmapLogoSVG from './assets/vulnmap-logo';
import vulnmapFaviconBase64 from './assets/vulnmap-favicon';
...
function rebrandHTMLOutput(data: string): string {
  ...
  data = data.replace(
    faviconReplaceRegex,
    `<link rel="shortcut icon" type="image/x-icon" href="${vulnmapFaviconBase64}" />`,
  );
  ...
  data = data.replace(
    titleReplaceRegex,
    `<title>Vulnmap IaC drift report</title>`,
  );
  ...
  data = data.replace(
    logoReplaceRegex,
    `<div id="brand_logo">${vulnmapLogoSVG}</div>`,
  );
