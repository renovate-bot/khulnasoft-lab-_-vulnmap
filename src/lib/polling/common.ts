...
export function handleProcessingStatus(
  ...
  if (response?.status === 'CANCELLED' || response?.status === 'ERROR') {
    throw new FailedToRunTestError(
      'Failed to process the project. Please run the command again with the `-d` flag and contact support@khulnasoft.khulnasoft.com with the contents of the output',
    );
  }
}
