...
import { IacTestResponse } from '../../lib/vulnmap-test/iac-test-result';
...
export function formatTestMeta(
  ...
  if (res.payloadType !== 'k8sconfig') {
    ...
    if (legacyRes.filesystemPolicy) {
      meta.push(
        chalk.bold(rightPadWithSpaces('Local Vulnmap policy: ', padToLength)) +
          chalk.green('found'),
      );
      if (
        ...
        meta.push(
          chalk.bold(
            rightPadWithSpaces('Local Vulnmap policy ignored: ', padToLength),
          ) + chalk.red('yes'),
        );
      }
    }
