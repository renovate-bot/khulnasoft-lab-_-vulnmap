...
import { SEVERITY } from '../../../vulnmap-test/legacy';
...
export function formatVulnmapIacTestTestData(
  vulnmapIacTestScanResult: Results | undefined,
  projectName: string,
  orgName: string,
): IacTestData {
  const resultsBySeverity = formatVulnmapIacTestScanResultNewOutput(
    vulnmapIacTestScanResult,
  );
  ...
  const allFilesCount = countFiles(vulnmapIacTestScanResult);
  const filesWithIssuesCount = countFilesWithIssues(vulnmapIacTestScanResult);
  const filesWithoutIssuesCount = allFilesCount - filesWithIssuesCount;
  const ignores = vulnmapIacTestScanResult
    ? vulnmapIacTestScanResult.metadata.ignoredCount
    : 0;

  let contextSuppressedIssueCount: number | undefined;
  const suppressedResults =
    vulnmapIacTestScanResult?.scanAnalytics?.suppressedResults;
...
function formatVulnmapIacTestScanResultNewOutput(
  vulnmapIacTestScanResult: Results | undefined,
): FormattedOutputResultsBySeverity {
  const resultsBySeverity = {} as FormattedOutputResultsBySeverity;

  if (vulnmapIacTestScanResult?.vulnerabilities) {
    vulnmapIacTestScanResult.vulnerabilities.forEach((vulnerability) => {
      ...
      resultsBySeverity[vulnerability.severity]!.push({
        issue: formatVulnmapIacTestScanVulnerability(vulnerability),
...
function formatVulnmapIacTestScanVulnerability(
