...
export function displayResult(
  ...
  if (res.ok && res.vulnerabilities.length === 0) {
    ...
    const nextStepsText = localPackageTest
      ? '\n\nNext steps:' +
        '\n- Run `vulnmap monitor` to be notified ' +
        'about new related vulnerabilities.' +
        '\n- Run `vulnmap test` as part of ' +
        'your CI/test.'
      : '';
    // user tested a package@version and got 0 vulns back, but there were dev deps
    // to consider
    // to consider
    const vulnmapPackageTestTip: string = !(
      options.docker ||
      localPackageTest ||
      options.dev
    )
      ? '\n\nTip: Vulnmap only tests production dependencies by default. You can try re-running with the `--dev` flag.'
      : '';
    ...
    return (
      ...
      (isCI()
        ? ''
        : dockerAdvice + nextStepsText + vulnmapPackageTestTip + dockerCTA)
    );
  }
