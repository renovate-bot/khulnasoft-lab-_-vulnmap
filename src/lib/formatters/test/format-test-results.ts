...
} from 'vulnmap-docker-plugin/dist';
...
export function extractDataToSendFromResults(
  ...
  return {
    ...
    stringifiedData, // this will be used to display either the Vulnmap or SARIF format JSON to stdout if --json or --sarif is set
...
export function dockerUserCTA(options) {
  if (options.isDockerUser) {
    return '\n\nFor more free scans that keep your images secure, sign up to Vulnmap at https://dockr.ly/3ePqVcp';
  }
  return '';
}
...
function getDockerSuggestionText(
  ...
  if (!options.file && !result?.docker?.baseImage) {
    tips.push(
      `Vulnmap wasn’t able to auto detect the base image, use \`--file\` option to get base image remediation advice.
Example: $ vulnmap container test ${options.path} --file=path/to/Dockerfile`,
    );
  }
  ...
  if (options.docker && result.targetFile && result.uniqueCount > 0) {
    tips.push(
      'Vulnmap found some vulnerabilities in your image applications (Vulnmap searches for these vulnerabilities by default). See https://vulnmap.co/app-vulns for more information.',
    );
  }
  ...
  return (
    ...
    tips
      ...
      .concat(
        ...
        } in the future, please run \`vulnmap config set disableSuggestions=true\``,
      )
      .join('\n\n')
  );
}
...
function getDockerRemediationDocsLink(dockerAdvice: string, config): string {
  ...
  return (
    ...
    chalk.white.underline(
      'https://docs.vulnmap.khulnasoft.com/products/vulnmap-container/getting-around-the-vulnmap-container-ui/base-image-detection',
    )
  );
}
