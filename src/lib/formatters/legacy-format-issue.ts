...
import { parsePackageString as vulnmapModule } from 'vulnmap-module';
import {
...
import { colorTextBySeverity } from '../../lib/vulnmap-test/common';
    ...
    const upgradePathsArray = uniq(
      vuln.list.map((v) => {
        ...
        if (shouldUpgradeItself) {
          ...
          // Example command: vulnmap test express@3
          const selfUpgradeInfo =
            v.upgradePath.length > 0
              ? ` (triggers upgrades to ${v.upgradePath.join(PATH_SEPARATOR)})`
              : '';
          const testedPackageName = vulnmapModule(v.upgradePath[0] as string);
          return (
