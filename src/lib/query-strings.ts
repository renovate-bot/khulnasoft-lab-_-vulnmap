...
export function getQueryParamsAsString(args: ArgsOptions[]): string {
  const utm_source = process.env.VULNMAP_UTM_SOURCE || 'cli';
  const utm_medium = process.env.VULNMAP_UTM_MEDIUM || 'cli';
  const utm_campaign =
    process.env.VULNMAP_UTM_CAMPAIGN || getIntegrationName(args) || 'cli';
  const utm_campaign_content =
    process.env.VULNMAP_UTM_CAMPAIGN_CONTENT || getIntegrationVersion(args);
