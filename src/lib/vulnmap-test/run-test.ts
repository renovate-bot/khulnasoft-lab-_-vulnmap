...
import { parsePackageString as moduleToObject } from 'vulnmap-module';
...
const debug = debugModule('vulnmap:run-test');
...
async function parseRes(
  ...
  if (!options['ignore-policy']) {
    ...
    const policy = await vulnmap.policy.loadFromText(res.policy);
...
async function assembleRemotePayloads(root, options): Promise<Payload[]> {
  ...
  // options.vulnEndpoint is only used by `vulnmap protect` (i.e. local filesystem tests)
