...
const debug = baseDebug('vulnmap-yarn-workspaces');
import * as lockFileParser from 'vulnmap-nodejs-lockfile-parser';
import {
...
export async function processYarnWorkspaces(
  ...
  const result: MultiProjectResultCustom = {
    plugin: {
      name: 'vulnmap-nodejs-yarn-workspaces',
  ...
  for (const directory of Object.keys(yarnTargetFiles)) {
    ...
    try {
      ...
      const project: ScannedProjectCustom = {
        ...
        depGraph: res as any,
        plugin: {
          name: 'vulnmap-nodejs-lockfile-parser',
          runtime: process.version,
        },
      };
