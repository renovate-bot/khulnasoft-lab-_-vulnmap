...
const debug = debugLib('vulnmap-code');
...
export async function logLocalCodeEngineVersion(lceUrl = ''): Promise<void> {
  ...
  try {
    ...
    if (body?.ok && body?.version) {
      debug(chalk.green(`Vulnmap Code Local Engine version: ${body.version}`));
      return;
    }
    ...
    if (body?.ok === false) {
      debug(
        chalk.red(
          `Vulnmap Code Local Engine health is not ok. statusCode:${statusCode}, version: ${body?.version}`,
        ),
      );
    ...
    debug(
      chalk.red(
        `Vulnmap Code Local Engine health check failed. statusCode:${statusCode}, version: ${JSON.stringify(
          body,
        )}`,
      ),
    );
  } catch (err) {
    debug('Vulnmap Code Local Engine health check failed.', err);
  } finally {
    if (isNonSecureHttp) {
