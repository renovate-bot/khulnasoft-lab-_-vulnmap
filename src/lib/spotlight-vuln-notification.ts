...
const debug = createDebug('vulnmap-spotlight-vuln-notification');

const spotlightVulnIds = ['VULNMAP-JAVA-ORGAPACHELOGGINGLOG4J-2314720'];
  ...
  try {
    if (foundSpotlightVulnsIds.length > 0) {
      ...
      for (const vulnId of spotlightVulnIds) {
        if (vulnId === 'VULNMAP-JAVA-ORGAPACHELOGGINGLOG4J-2314720') {
          ...
          message +=
            ...
            EOL +
            `  - ${config.PUBLIC_VULN_DB_URL}/vuln/VULNMAP-JAVA-ORGAPACHELOGGINGLOG4J-2314720` +
            EOL +
