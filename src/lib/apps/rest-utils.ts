...
const debug = Debug(VULNMAP_APP_DEBUG);
...
function errorsToDisplayString(errRes: IRestErrorResponse): string {
  const resString = `Uh oh! an error occurred while trying to create the Vulnmap App.
Please run the command with '--debug' or '-d' to get more information`;
  ...
  errRes.errors.forEach((e) => {
    ...
    return `Uh oh! an error occured while trying to create the Vulnmap App.
...
export function handleCreateAppRes(res: ICreateAppResponse): string {
  ...
  return `Vulnmap App created successfully!
