...
import { Global } from '../../../cli/src/cli/args';
import { Payload } from './types';
import { getVersion } from '../version';
import * as https from 'https';
import * as http from 'http';

const debug = debugModule('vulnmap:req');
const vulnmapDebug = debugModule('vulnmap');
...
function setupRequest(payload: Payload) {
  ...
  payload.headers['x-vulnmap-cli-version'] = versionNumber;
  ...
  if (body && !noCompression) {
    ...
    vulnmapDebug('sending request to:', payload.url);
    vulnmapDebug('request body size:', json.length);
    vulnmapDebug('gzipped request body size:', data.length);

    let callGraphLength: number | null = null;
    if (body.callGraph) {
      callGraphLength = JSON.stringify(body.callGraph).length;
      vulnmapDebug('call graph size:', callGraphLength);
    }
  ...
  if (
    ...
    process.env.VULNMAP_HTTP_PROTOCOL_UPGRADE !== '0'
  ) {
  ...
  if (proxyUri) {
    vulnmapDebug('using proxy:', proxyUri);
    bootstrap({
      environmentVariableNamespace: '',
    });
  } else {
    vulnmapDebug('not using proxy');
  }
