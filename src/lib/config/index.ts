import * as vulnmapConfig from 'vulnmap-config';
...
// TODO: fix the types!
const config = (vulnmapConfig.loadConfig(
  __dirname + '/../..',
) as unknown) as Config;
const defaultApiUrl = 'https://api.vulnmap.khulnasoft.com';

const configDefinedApiUrl = userConfig.get('endpoint');
const envvarDefinedApiUrl = process.env.VULNMAP_API;

const vulnmapApiBaseUrl = getBaseApiUrl(
  defaultApiUrl,
  envvarDefinedApiUrl,
  configDefinedApiUrl,
);
config.API = getV1ApiUrl(vulnmapApiBaseUrl);
...
config.PUBLIC_VULN_DB_URL = 'https://security.vulnmap.khulnasoft.com';
