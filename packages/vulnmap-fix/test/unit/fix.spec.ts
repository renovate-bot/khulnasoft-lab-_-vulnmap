import * as pipenvPipfileFix from '@khulnasoft/fix-pipenv-pipfile';

import * as vulnmapFix from '../../src';
import { generateEntityToFix } from '../helpers/generate-entity-to-fix';

jest.mock('@khulnasoft/fix-pipenv-pipfile');

describe('Vulnmap fix', () => {
  beforeAll(() => {
  ...
  it('Vulnmap fix returns results for supported type', async () => {
    // Arrange
    ...
    // Act
    const res = await vulnmapFix.fix([projectTestResult], {
      quiet: true,
      stripAnsi: true,
    });
  ...
  it('Vulnmap fix returns results for supported type in dryRun mode (no write)', async () => {
    // Arrange
    const projectTestResult = generateEntityToFix(
      'pip',
      'requirements.txt',
    ...
    const writeFileSpy = jest.spyOn(projectTestResult.workspace, 'writeFile');
    // Act
    await vulnmapFix.fix([projectTestResult], {
      quiet: true,
      dryRun: true,
    });
  ...
  it('Vulnmap fix returns results for supported & unsupported type', async () => {
    // Arrange
    const projectTestResult = generateEntityToFix(
      'pip',
      'requirements.txt',
    ...
    // Act
    const res = await vulnmapFix.fix(
  ...
  it('Vulnmap fix returns results as expected', async () => {
    // Arrange
  ...
  it('Vulnmap fix returns results as expected when 1 fails to fix', async () => {
    // Arrange
    const txtProdProjectTestResult = generateEntityToFix(
      'pip',
      'prod.txt',
  ...
  it('Vulnmap fix returns results as expected when remediation data is empty', async () => {
    // Arrange
    const txtProdProjectTestResult = generateEntityToFix(
      'pip',
      'prod.txt',
    ...
    // Act
    const res = await vulnmapFix.fix([txtProdProjectTestResult], {
      quiet: true,
      stripAnsi: true,
    });
    // Assert
...
describe('groupEntitiesPerScanType', () => {
  it('correctly groups related entities per handler type (pip)', () => {
    ...
    // Act
    const res = vulnmapFix.groupEntitiesPerScanType([
      txtProdProjectTestResult,
      txtDevProjectTestResult,
      pipfileProjectTestResult,
    ]);
...
describe('Error handling', () => {
  it('Vulnmap fix returns error when called with unsupported type', async () => {
    // Arrange
    const projectTestResult = generateEntityToFix(
      'npm',
      'package.json',
  ...
  it('Vulnmap fix returns error when manifest can not be parsed', async () => {
    // Arrange
    const projectTestResult = generateEntityToFix(
      'pip',
      'requirements.txt',
