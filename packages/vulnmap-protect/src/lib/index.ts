...
import { extractPatchMetadata } from './vulnmap-file';
...
async function protect(projectFolderPath: string) {
  ...
  const vulnmapFilePath = path.resolve(projectFolderPath, '.vulnmap');

  if (!fs.existsSync(vulnmapFilePath)) {
    console.log('No .vulnmap file found.');
    sendAnalytics({
      type: ProtectResultType.NO_VULNMAP_FILE,
    });
    return;
  }

  const vulnmapFileContents = fs.readFileSync(vulnmapFilePath, 'utf8');
  const vulnmapFilePatchMetadata = extractPatchMetadata(vulnmapFileContents);
  const vulnIdAndPackageNames: VulnIdAndPackageName[] = vulnmapFilePatchMetadata;
  const targetPackageNames = [
    ...new Set(vulnmapFilePatchMetadata.map((vpn) => vpn.packageName)), // get a list of unique package names by converting to Set and then back to array
  ];
  ...
  console.log('Applied Vulnmap patches.');
