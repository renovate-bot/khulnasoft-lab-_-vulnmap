...
const debug = debuglog('@khulnasoft' + __filename);

/**
 * Modifies the given fixture's package.json to use a locally built
 * @khulnasoft/protect tarball instead of the one in NPM Registry.
 *
...
const useLocalPackage = async (projectPath: string) => {
  ...
  if (packageJson.dependencies) {
    packageJson.dependencies['@khulnasoft/protect'] = path.resolve(
      workspaceRoot,
      tarballName,
    );
  }
