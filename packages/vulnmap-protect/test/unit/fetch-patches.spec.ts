...
describe('fetchPatches', () => {
  it('can fetch patches for valid request params', async () => {
    ...
    await expect(
      fetchPatches('VULNMAP-JS-LODASH-567746', 'lodash', '4.17.15'),
    ).resolves.toEqual([
      {
  ...
  it('throws when version not valid semver', async () => {
    await expect(
      fetchPatches('VULNMAP-JS-LODASH-567746', 'lodash', 'not-valid-semver'),
  ...
  it('returns empty array when no patches found for vuln/version', async () => {
    await expect(
      fetchPatches('VULNMAP-JS-SSRI-1246392', 'ssri', '5.2.3'),
    ).resolves.toEqual([]);
  });
});
...
describe('getAllPatches', () => {
  ...
  it('throws when a vulnId is not found', async () => {
    const vulnIdAndPackageNames = [
      ...
      {
        vulnId: 'VULNMAP-JS-VULN_ID_DOES_NOT_EXIST',
      ...
      patches: [
        {
          patchableVersions: '>=4.14.2',
          urls: [
            'https://vulnmap-patches.s3.amazonaws.com/npm/lodash/20200430/lodash_0_0_20200430_6baae67d501e4c45021280876d42efe351e77551.patch',
            'https://vulnmap-patches.s3.amazonaws.com/npm/lodash/20190702/lodash_20190702_0_0_1f8ea07746963a535385a5befc19fa687a627d2b.patch',
          ],
        },
        {
          patchableVersions: '>=4.15.1',
          urls: [
            'https://vulnmap-patches.s3.amazonaws.com/npm/lodash/20180130/20180130_0_0_lodash_d8e069cc3410082e44eb18fcf8e7f3d08ebe1d4a.patch',
            'https://vulnmap-patches.s3.amazonaws.com/npm/axios/20190424/axios_20190424_0_0_79a1ed204f4c2971b573edde816b3a43b25240da.patch', // yes this is for axios and makes no sense - it's a contrived scenario
          ],
        },
      ],
    };
