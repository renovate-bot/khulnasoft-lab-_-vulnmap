...
import (
	...
	"github.com/khulnasoft-lab/cli-extension-dep-graph/pkg/depgraph"
	"github.com/khulnasoft-lab/cli-extension-iac-rules/iacrules"
	"github.com/khulnasoft-lab/cli-extension-sbom/pkg/sbom"
	"github.com/khulnasoft-lab/container-cli/pkg/container"
	...
	vulnmapls "github.com/khulnasoft-lab/vulnmap-ls/ls_extension"
...
func getDebugLevel(config configuration.Configuration) zerolog.Level {
	...
	if loglevelString := config.GetString("vulnmap_loglevel"); loglevelString != "" {
		var err error
...
func initApplicationConfiguration(config configuration.Configuration) {
	config.AddAlternativeKeys(configuration.AUTHENTICATION_TOKEN, []string{"vulnmap_cfg_api", "api"})
	config.AddAlternativeKeys(configuration.AUTHENTICATION_BEARER_TOKEN, []string{"vulnmap_docker_token"})
	...
	config.AddAlternativeKeys(configuration.ANALYTICS_DISABLED, []string{strings.ToLower(constants.VULNMAP_ANALYTICS_DISABLED_ENV), "vulnmap_cfg_disable_analytics", "disable-analytics", "disable_analytics"})
	...
	if !ok {
		...
		for _, key := range alternativeBearerKeys {
			hasPrefix := strings.HasPrefix(key, "vulnmap_")
			if hasPrefix {
...
func createCommandsForWorkflows(rootCommand *cobra.Command, engine workflow.Engine) {
	...
	for i := range workflowIdList {
		...
		// last parentCommand is the last level of the command, e.g. in the vulnmap iac capture workflow, it would be 'capture'
...
func MainWithErrorCode() int {
	...
	engine.AddExtensionInitializer(vulnmapls.Init)
	...
	networkAccess.AddHeaderField("x-vulnmap-cli-version", cliv2.GetFullVersion())
	networkAccess.AddHeaderField(
		"User-Agent",
		networking.UserAgent(
			networking.UaWithConfig(globalConfiguration),
			networking.UaWithApplication("vulnmap-cli", cliv2.GetFullVersion()),
