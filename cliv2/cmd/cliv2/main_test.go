...
func Test_MainWithErrorCode(t *testing.T) {
	...
	os.Args = []string{"vulnmap", "--version"}
...
func Test_initApplicationConfiguration_DisablesAnalytics(t *testing.T) {
	t.Run("via VULNMAP_DISABLE_ANALYTICS (true)", func(t *testing.T) {
		c := configuration.NewInMemory()
		assert.False(t, c.GetBool(configuration.ANALYTICS_DISABLED))

		c.Set("VULNMAP_DISABLE_ANALYTICS", "true")
		initApplicationConfiguration(c)

		assert.True(t, c.GetBool(configuration.ANALYTICS_DISABLED))
	})
	t.Run("via VULNMAP_DISABLE_ANALYTICS (1)", func(t *testing.T) {
		c := configuration.NewInMemory()
		assert.False(t, c.GetBool(configuration.ANALYTICS_DISABLED))

		c.Set("VULNMAP_DISABLE_ANALYTICS", "1")
		initApplicationConfiguration(c)

		assert.True(t, c.GetBool(configuration.ANALYTICS_DISABLED))
	})
	t.Run("via VULNMAP_CFG_DISABLE_ANALYTICS (true)", func(t *testing.T) {
		c := configuration.NewInMemory()
		assert.False(t, c.GetBool(configuration.ANALYTICS_DISABLED))

		c.Set("VULNMAP_CFG_DISABLE_ANALYTICS", "true")
		initApplicationConfiguration(c)

		assert.True(t, c.GetBool(configuration.ANALYTICS_DISABLED))
	})
	t.Run("via VULNMAP_CFG_DISABLE_ANALYTICS (1)", func(t *testing.T) {
		c := configuration.NewInMemory()
		assert.False(t, c.GetBool(configuration.ANALYTICS_DISABLED))

		c.Set("VULNMAP_CFG_DISABLE_ANALYTICS", "1")
		initApplicationConfiguration(c)
