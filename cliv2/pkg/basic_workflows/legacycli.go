...
import (
	...
	pkg_utils "github.com/khulnasoft-lab/go-application-framework/pkg/utils"
...
func legacycliWorkflow(
	...
	if _, exists := os.LookupEnv(constants.VULNMAP_ANALYTICS_DISABLED_ENV); !exists && analyticsDisabled {
		env := []string{constants.VULNMAP_ANALYTICS_DISABLED_ENV + "=1"}
	...
	if oauthIsAvailable {
		...
		if oauth := config.GetString(auth.CONFIG_KEY_OAUTH_TOKEN); len(oauth) > 0 {
			...
			if _, ok := envMap[constants.VULNMAP_OAUTH_ACCESS_TOKEN_ENV]; !ok {
				env := []string{constants.VULNMAP_OAUTH_ACCESS_TOKEN_ENV + "=randomtoken"}
				cli.AppendEnvironmentVariables(env)
