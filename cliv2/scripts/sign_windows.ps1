param (
    ...
    [string]$PRODUCT_NAME = "vulnmap_windows_amd64.exe"   # Default name of the product (executable)
)
...
& $SIGNTOOL sign /sha1 $env:SM_CODE_SIGNING_CERT_SHA1_HASH /tr http://timestamp.digicert.com /td SHA256 /fd SHA256 /d "Vulnmap CLI" /du "https://vulnmap.khulnasoft.com" /v $APP_PATH_UNSIGNED
if ($LASTEXITCODE) {
    exit $LASTEXITCODE
}
