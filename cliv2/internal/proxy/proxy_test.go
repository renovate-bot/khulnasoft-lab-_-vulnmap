...
func Test_canGoThroughProxy(t *testing.T) {
	...
	res, err := proxiedClient.Get("https://static.vulnmap.khulnasoft.com/cli/latest/version")
	if err != nil {
		t.Fatal(err)
	}
...
func Test_SetUpstreamProxy(t *testing.T) {
	...
	testUrl, _ := url.Parse("http://www.vulnmap.khulnasoft.com")
...
func Test_appendExtraCaCert(t *testing.T) {
	...
	os.Setenv(constants.VULNMAP_CA_CERTIFICATE_LOCATION_ENV, file.Name())
	...
	// cleanup
	os.Unsetenv(constants.VULNMAP_CA_CERTIFICATE_LOCATION_ENV)
	os.Remove(file.Name())
}
