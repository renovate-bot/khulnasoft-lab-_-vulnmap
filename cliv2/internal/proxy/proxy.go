...
const (
	PROXY_REALM    = "vulnmapcli_realm"
	PROXY_USERNAME = "vulnmapcli"
)
...
func NewWrapperProxy(config configuration.Configuration, cliVersion string, debugLogger *log.Logger) (*WrapperProxy, error) {
	...
	certName := "vulnmap-embedded-proxy"
	...
	certFile, err := os.CreateTemp(tmpDirectory, "vulnmap-cli-cert-*.crt")
	if err != nil {
	...
	if extraCaCertFile, ok := os.LookupEnv(constants.VULNMAP_CA_CERTIFICATE_LOCATION_ENV); ok {
