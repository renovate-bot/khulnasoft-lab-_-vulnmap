...
if grep -q "github.com/khulnasoft-lab/vulnmap-iac-parsers ${VERSION}" "go.mod"; then
  echo "
  ...
  "
  go get github.com/khulnasoft-lab/vulnmap-iac-parsers@"${VERSION}"
  if [ $? -eq 0 ]; then
      echo "Downloaded version ${VERSION} of the synk-iac-parsers"
  else
    printError "Download of version ${VERSION} of the vulnmap-iac-parsers failed. Please check the error above for details."
    exit 1
  fi
fi
