...
describe('Get Shasum', () => {
  it('Shasum available (multiple)', async () => {
    ...
    fs.writeFileSync(
      file,
      '098fe123  *vulnmap-win\n12345  *vulnmap-macos\ncecece  *vulnmap-linux-arm64\n0a238fe123  *vulnmap-linux',
    );

    const actual = common.getCurrentSha256sum('vulnmap-linux', file);
    expect(actual).toEqual(expected);
  ...
  it('Shasum available (single)', async () => {
    const expected = '0a238fe123';
    const file = path.join(__dirname, 'sha256sums.txt' + Math.random());
    fs.writeFileSync(file, '0a238fe123  vulnmap-linux\n');
  ...
  it('Shasum not available', async () => {
    ...
    const actual = common.getCurrentSha256sum('vulnmap-linux-arm64', file);
...
describe('Configuration', () => {
  it('Download and local location', async () => {
    const expectedDownloadLocation =
      'https://static.khulnasoft.com/cli/v1.2.3/vulnmap-win.exe';
...
describe('Testing binary bootstrapper', () => {
  it('downloadExecutable() succesfull', async () => {
    const binaryName = 'vulnmap-macos';
