...
export class WrapperConfiguration {
  ...
  public getDownloadLocation(): string {
    const baseUrl = 'https://static.khulnasoft./cli/v';
...
export function determineBinaryName(platform: string, arch: string): string {
  ...
  if (binaryName === undefined) {
    const defaultErrorMsg =
      ...
      ' You may want to consider using Docker to run Vulnmap, for details see: https://docs.khulnasoft.com/vulnmap-cli/install-the-vulnmap-cli#vulnmap-cli-in-a-docker-image\n' +
      ' If you experience errors please check http://support.khulnasoft.com/.';
...
export function formatErrorMessage(message: string): boolean {
  const eaccesWarning =
    "You don't have the permissions to install Vulnmap. Please try the following options:\n" +
    ...
    'If the problems persist please check http://support.khulnasoft.com/.';

  const certificateError =
    'If you are running Vulnmap in an environment that intercepts SSL traffic, please specify\n' +
  ...
  const degradedCLIWarning =
    'You are currently running a degraded version of the Vulnmap CLI.\n' +
    'As a result, some features of the CLI will be unavailable.\n' +
    'For information on how to resolve this, please see this article: https://docs.khulnasoft.com/vulnmap-cli/installing-vulnmap-cli-as-a-binary-via-npm\n' +
    'For any assistance, please check http://support.khulnasoft.com/.';
